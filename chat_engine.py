...
def _calculate_gochara(planets_data: list, planet_houses: dict, moon_rasi: str) -> dict:
    ...
    moon_rasi_n = unicodedata.normalize('NFC', moon_rasi)
    moon_rasi_idx = rasi_names_n.index(moon_rasi_n) if moon_rasi_n in rasi_names_n else -1
    ...
    for p in planets_data:
        if p.get("name") == "লগ্ন":
            ...
            lagna_rasi_n = unicodedata.normalize('NFC', lagna_rasi)
            if lagna_rasi_n in rasi_names_n:
                lagna_rasi_idx = rasi_names_n.index(lagna_rasi_n)
            break
